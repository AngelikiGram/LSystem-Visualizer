...
new_func = '''def _expand_tree_to_level(raw_string, target_level, enrich_scale=0):
    ...
    def generate_random_branch(d_remaining, parent_total_f):
        ...
        t_base = random.randint(0, 4)
        p_base = random.randint(0, 4)
...
pattern = r'def _expand_tree_to_level\(.*?def _enrich_at_depth'
...
print('Success!' if text != new_text else 'Failed to replace')
